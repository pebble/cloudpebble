...
def load_screenshots(request, project_id, test_id):
    ...
    return {"screenshots": [make_screenshot_dict(screenshot, project_id) for screenshot in screenshots]}
...
def sync_screenshots(request, project_id, test_id):
    ...
    try:
        ...
        # I can't remember which FloatingPointError this was intended to catch.
        raise BadRequest(str(e))

    current_screenshot_sets = ScreenshotSet.objects.filter(test=test)
    return {"screenshots": [make_screenshot_dict(screenshot, project_id) for screenshot in current_screenshot_sets]}
