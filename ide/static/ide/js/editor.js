CloudPebble.Editor = (function() {
    ...
    var add_test_file = function(file) {
        file.target = 'test';
        CloudPebble.Sidebar.AddTestFile(file, function() {
            edit_source_file(file);
        });
    ...
    var rename_file = function(file, new_name) {
        ...
        return Ajax.Post("/ide/project/" + PROJECT_ID + "/" + url_kind + "/" + file.id + "/rename", {
            old_name: file.name,
            new_name: new_name,
    ...
    var edit_source_file = function(file, show_ui_editor) {
        ...
        // Open it.
        return Ajax.Get('/ide/project/' + PROJECT_ID + '/' + url_kind + '/' + file.id + '/load').then(function(data) {
            var screenshot_pane;
            ...
            else if (file.target == "test") {
                file_kind = 'monkey';
                file_mode = 'MonkeyScript'
            }
            ...
            var language_has_autocomplete = file_kind_in(['c', 'monkey']);
            ...
            var check_safe = function() {
                return Ajax.Get('/ide/project/' + PROJECT_ID + '/' + url_kind + '/' + file.id + '/is_safe?modified=' + file.lastModified).then(function(data) {
                    if(!data.safe) {
                        if(was_clean) {
            ...
            code_mirror.on('change', function() {
                if(was_clean) {
                    CloudPebble.Sidebar.SetIcon(sidebar_id, 'edit');
                    was_clean = false;
                    ++unsaved_files;
                }
            });
            ...
            var mark_clean = function() {
                was_clean = true;
                --unsaved_files;
                CloudPebble.Sidebar.ClearIcon(sidebar_id);
            };
            ...
            var save = function() {
                ...
                return Ajax.Post("/ide/project/" + PROJECT_ID + "/" + url_kind +  "/" + file.id + "/save", {
            ...
            delete_btn.click(function() {
                ...
                CloudPebble.Prompts.Confirm(interpolate(fmt, file, true), gettext("This cannot be undone."), function() {
                    ...
                    Ajax.Post("/ide/project/" + PROJECT_ID + "/" + url_kind + "/" + file.id + "/delete").then(function(data) {
                        CloudPebble.Sidebar.DestroyActive();
                        delete project_source_files[file.file_path];
                        CloudPebble.Sidebar.Remove(sidebar_id);
    ...
    function create_remote_test(params) {
        if (_.isString(params)) {
            params = {name: params};
        }
    ...
    function create_test_file() {
        ...
        CloudPebble.Prompts.Prompt(gettext("Create new test file"), "Test name", "", "", function(value, prompt_action) {
    ...
    function go_to(file_info, line, ch) {
        ...
        // Otherwise, look up the file by its keys.
